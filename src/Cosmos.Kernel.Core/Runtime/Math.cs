...
// ARM64 transcendental functions ported from fdlibm via Cosmos Gen2
// (Cosmos/source/Cosmos.System2_Plugs/System/MathImpl.cs).
// x64 uses x87 FPU assembly stubs in Cosmos.Kernel.Native.X64/Runtime/Runtime.asm.
...
internal static class Math
{
    ...
    {
        // Round to nearest integer (banker's rounding not needed here)
    ...
    // =========================================================================
    // ARM64 transcendental math — no hardware support (unlike x87 on x64).
    // Ported from fdlibm (Sun Microsystems) via Cosmos Gen2 MathImpl.cs.
    // On x64 these symbols are provided by Runtime.asm using x87 FPU.
    // =========================================================================
    ...
    {
        // Taylor: x - x^3/3! + x^5/5! - ... (9 terms, up to x^17/17!)
        double x2 = x * x;
        double term = x;
        double sum = x;
    ...
    {
        // Taylor: 1 - x^2/2! + x^4/4! - ... (9 terms, up to x^16/16!)
        double x2 = x * x;
        double term = 1.0;
        double sum = 1.0;
    ...
    internal static double sin(double x)
    {
        ...
        // Reduce to [0, |x|], remember sign
        double sign = 1.0;
        if (x < 0) { sign = -1.0; x = -x; }

        // j = round(x / (PI/2)), r = x - j*(PI/2)
    ...
    internal static double exp(double x)
    {
        ...
        if (k >= -1021)
        {
            ...
            _y += (long)k << 52;            /* add k to y's exponent */
    ...
    [RuntimeExport("expf")]
    internal static float expf(float x) => (float)exp(x);
    ...
    [RuntimeExport("logf")]
    internal static float logf(float x) => (float)log(x);
    ...
    /* atan(0.5)hi/lo, atan(1.0)hi/lo, atan(1.5)hi/lo, atan(inf)hi/lo */
    ...
    [RuntimeExport("atanf")]
    internal static float atanf(float x) => (float)atan(x);
    ...
    internal static double asin(double x)
    {
        ...
        const double pio2_hi = 1.57079632679489655800e+00; /* 0x3FF921FB, 0x54442D18 */
        const double pio2_lo = 6.12323399573676603587e-17; /* 0x3C91A626, 0x33145C07 */
        const double pio4_hi = 7.85398163397448278999e-01; /* 0x3FE921FB, 0x54442D18 */
        const double pS0 = 1.66666666666666657415e-01;     /* 0x3FC55555, 0x55555555 */
        const double pS1 = -3.25565818622400915405e-01;    /* 0xBFD4D612, 0x03EB6F7D */
        const double pS2 = 2.01212532134862925881e-01;     /* 0x3FC9C155, 0x0E884455 */
        const double pS3 = -4.00555345006794114027e-02;    /* 0xBFA48228, 0xB5688F3B */
        const double pS4 = 7.91534994289814532176e-04;     /* 0x3F49EFE0, 0x7501B288 */
        const double pS5 = 3.47933107596021167570e-05;     /* 0x3F023DE1, 0x0DFDF709 */
        const double qS1 = -2.40339491173441421878e+00;    /* 0xC0033A27, 0x1C8A2D4B */
        const double qS2 = 2.02094576023350569471e+00;     /* 0x40002AE5, 0x9C598AC8 */
        const double qS3 = -6.88283971605453293030e-01;    /* 0xBFE6066C, 0x1B8D0159 */
        const double qS4 = 7.70381505559019352791e-02;     /* 0x3FB3B8C5, 0xB12E9282 */
        ...
        {
            w = s;
            w = SetLowWord(w, 0);                   /* truncate to ~26 bits */
            c = (t - w * w) / (s + w);
            r = p / q;
    ...
    internal static double acos(double x)
    {
        ...
        else
        {
            ...
            s = sqrt(z);
            df = s;
            df = SetLowWord(df, 0);         /* truncate to ~26 bits */
            c = (z - df * df) / (s + df);
            p = z * (pS0 + z * (pS1 + z * (pS2 + z * (pS3 + z * (pS4 + z * pS5)))));
            q = 1 + z * (qS1 + z * (qS2 + z * (qS3 + z * qS4)));
            r = p / q;
    ...
    internal static double pow(double x, double y)
    {
        ...
        /* Integer exponent fast path — gives exact results for small n */
        if (y == trunc(y) && y >= -64 && y <= 64)
        {
            ...
            /* apply sign for negative base with odd integer exponent */
            if (x < 0 && ((long)y & 1) != 0)
            {
                result = -result;
            }
